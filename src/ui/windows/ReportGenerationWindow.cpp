...
ReportGenerationWindow::ReportGenerationWindow(QWidget* parent, Model* model,
    ...
    ui->title_textEdit->setPlainText(tr("3D Model Inventory Report"));
    ui->subtitle_textEdit->setPlainText(
        tr("Contains %1 selected model(s).").arg(model->getSelectedModels().size()));
    ui->version_textEdit->setPlainText(
        tr("Generated by CADventory %1").arg(QCoreApplication::applicationVersion()));
    ui->outputDirectory_textEdit->setPlainText(
        QString::fromStdString(dotCadventory) + "/data/gist_reports/report.pdf");
    ui->pauseReport_pushButton->hide(); // Report cancellation is not supported by the queued worker.
...
void ReportGenerationWindow::onGenerateReportButtonClicked() {
    ...
    const auto jobsDir = model->getHiddenPaths().jobsDir();
    ...
    ui->fileInProcess_label->setText(tr("Report queued for processing."));
